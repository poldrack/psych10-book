...
**Binary numbers**. The simplest are binary numbers -- that is, zero or one.  We will often use binary numbers to represent whether something is true or false, or present or absent.  For example, I might ask 10 people if they have ever experienced a migraine headache.  We can put their answers into an R variable, using the ```c()``` function to create a list of values.  Then we can simply type the name of the variable to print out its value:
...
We can also turn our list of truth values into integers explicitly using the ```as.integer()``` function in R:

```{r}
# create integer values from truth values using as.integer()

everHadMigraineBinary <- as.integer(everHadMigraineTruth)
