...
Finally, we want to plot the points in separate colors depending on whether their gender.  We do this by simply adding a *color* keyword to the aesthetics, which tells the `geom_point()` function to separately color points by Gender.  This is shown in Figure \@ref(fig:colorPoints).  This figure also shows an example of how we can include multiple geometry layers in a single figure -- in this case, we use `geom_smooth()` to plot the lines that best describe relation between height and weight, separately by gender.  The shading around the lines reflects our confidence in the estimate at that point.

```{r colorPoints,fig.cap='Scatterplot with points separately colored by Gender.',fig.width=4,fig.height=4,out.height='50%'}
ggplot(NHANES_sample,aes(x=Height,y=Weight,color=Gender)) +
  geom_point() + 
