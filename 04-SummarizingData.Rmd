...
```{r salesContract,echo=FALSE,fig.cap="A Sumerian tablet from the Louvre, showing a sales contract for a house and field.  Public domain, via Wikimedia Commons.",fig.width=4,fig.height=4,out.height='50%'}
...
```{r sleepHist,echo=FALSE,fig.cap="Left: Histogram showing the number (left) and proportion (right) of people reporting each possible value of the SleepHrsNight variable.",fig.width=8,fig.height=4,out.height='50%'}
...
```{r ageHist,echo=FALSE,fig.cap="A histogram of the Age variable in NHANES.",fig.width=4,fig.height=4,out.height='50%'}
...
```{r heightHist,echo=FALSE,fig.cap="A histogram of height values from NHANES.",fig.width=4,fig.height=4,out.height='50%'}
NHANES %>% 
  select(Height) %>% 
  drop_na() %>% 
  ggplot(aes(Height)) +
...
```{r heightHistSep,echo=FALSE,fig.cap="Histogram of heights for NHANES. Left: values plotted separately for children (blue) and adults (red).  Right: values for adults only.",fig.width=8,fig.height=4,out.height='50%'}
...
```{r AdultHistSmallBins,echo=FALSE,fig.cap="A histogram of height values from NHANES, showing the frequency of values down to one tenth of a centimeter.",fig.width=4,fig.height=4,out.height='50%'}
NHANES_adult %>% 
  drop_na(Height) %>% 
  ggplot(aes(Height)) +
  geom_histogram(aes(y = ..density..), binwidth = .1)
