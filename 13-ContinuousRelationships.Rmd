...
# Create Lorenz curve for a population with normally distributed income.
...
# Create Lorenz curve for a maximally inequal population
...
```{r shuffleCorr,echo=FALSE,fig.cap="Histogram of correlation values under the null hypothesis, obtained by shuffling values. Observed value is denoted by blue line."}
...
```{r outlierCorr, echo=FALSE,fig.cap="An example of the effects of outliers on correlation.  Without the outlier the remainder of the datapoints have a perfect negative correlation, but the single outlier changes the correlation value to highly positive."}
n <- 10
set.seed(1234)
...
```{r latentCausalGraph, echo=FALSE,fig.cap="A graph showing the same causal relationships as above, but now also showing the latent variable (knowledge) using a square box."}
