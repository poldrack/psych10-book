...
NHANES_sample <- 
  ...
  mutate(
    TVHrsNum=recode(TVHrsDay,'More_4_hr'=5,
                         '4_hr'=4,'2_hr'=2,
...
```{r PotTVViolin,echo=FALSE,fig.cap='Violin plot showing distributions of TV watching separated by regular marijuana use.'}
...
t.test(TVHrsNum~RegularMarij,data=NHANES_sample,
       var.equal=TRUE)
```
...
```{r ttestFig,echo=FALSE,fig.cap="Violin plots showing data for each group, with a blue line connecting the predicted values for each group, computed on the basis of the results of the linear model."}
ggplot(NHANES_sample,aes(RegularMarij,TVHrsNum)) +
  geom_violin() +
...
The most commonly used effect size for a comparison between two means is Cohen's d, which (as you may remember from the earlier chapter on effect sizes) is an expression of the effect in terms of standard error units.  For the t-test estimated using the general linear model outlined above, this is expressed as:
...
```{r BPfig, echo=FALSE,fig.cap='Violin plot of systolic blood pressure on first and second recording, from NHANES.'}
...
We see that there does not seem to be much of a difference overall between time points (about one point). First let's test for a difference using an independent samples t-test, which ignores the fact that pairs of data points come from the the same individuals.   

```{r}
t.test(BPsys~timepoint,data=NHANES_sample_tidy,
...
```{r BPLinePlot,echo=FALSE,fig.cap='Violin plot of systolic BP on each recording, with lines connecting the two data points for each individual.'}
...
```{r BPDiffHist,echo=FALSE,fig.cap="Histogram of difference scores between first and second BP measurement."}
...
You can see that this shows us the same p-value as the paired t-test computed using the ```t.test()``` function.
...
```{r DrugTrial, echo=FALSE,fig.cap='Box plots showing blood pressure for three different groups in our clinical trial.'}
...
```{r FDist, echo=FALSE,fig.cap='F distributions under the null hypothesis, for different values of degrees of freedom.'}
fdata <- 
