...
One of the most common questions that we want to ask in statistics is whether there is a difference between the means of two different groups.  Let's say that we would like to know whether smokers and nonsmokers differ in their blood pressure.  We can ask this question using the NHANES dataset; let's take a sample of 200 individuals from the dataset and test whether mean blood pressure differs between these two groups.

```{r}
NHANES_sample=NHANES %>% 
  drop_na(SmokeNow,BPSysAve) %>%
  sample_n(200)


```

```{r SmokingBPViolin,fig.cap='Violin plot showing distributions of systolic blood pressure separately for smokers and nonsmokers.'}
ggplot(NHANES_sample,aes(SmokeNow,BPSysAve)) +
...
```{r}
t.test(BPSysAve~SmokeNow,data=NHANES_sample,var.equal=TRUE)
```
In this case we see that there is a statstically significant difference between groups, though it goes in a direction that we might not have predicted -- smokers actually have *lower* blood pressure than non-smokers.
...
```{r}
s=summary(lm(BPSysAve~SmokeNow,data=NHANES_sample))
s
```
...
ggplot(NHANES_sample,aes(SmokeNow,BPSysAve)) +
  geom_violin() +
  annotate('segment',x=1,y=s$coefficients[1,1],xend=2,yend=s$coefficients[1,1]+s$coefficients[2,1])
...
The most commonly used effect size for a comparison between two means is Cohen's d, which you may remember from the chapter on effect sizes is an expression of the effect in terms of standard error units.  For the t-test estimated using the general linear model outlined above, this is expressed as:

$$
d = \frac{\hat{beta_1}}{SE_{residual}}
$$
We can obtain these values from the analysis output above, giving us a d = `r I(s$coefficients[2,1]/s$sigma)`, which we would generally interpret as a moderately strong effect.

We can also compute $R^2$ for this analysis, which tells us how much variance in blood pressure is accounted for by smoking.  This value (which is reported in the summary of the lm() analysis) is 0.0008, which tells us that while the effect may be statistically significant, it accounts for relatively little of the variance in blood pressure.
...
```{r}
bf = ttestBF(formula = 
    BPSysAve~SmokeNow,data=NHANES_sample)
bf
```
