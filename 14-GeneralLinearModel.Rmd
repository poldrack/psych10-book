...
In order to get the standard error for a specific regression parameter estimate, $SE_{\beta_x}$, we need to rescale the sum of squared errors by the variability of the x variable, take the square root (to create the scaled standard deviation), and then divide by the square root of the number of degrees of freedom (n - 2) to generate a standard error:

$$
SE_{\beta_x} = \frac{1}{\sqrt{n-2}}*\sqrt{\frac{SS_{error}}{\sum{(x_i - \bar{x})^2}}}
$$
...
Another way to see the effects of overfitting is to look at what happens if we randomly shuffle the values of the weight variable. Randomly shuffiling the value should make it impossible to predict weight from the other variables, because they should have no systematic relationship.
...
```{r crossvalidation,echo=FALSE,fig.cap="A schematic of the  crossvalidation procedure.",fig.height=4,out.height='30%'}
