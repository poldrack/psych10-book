...
mean_rsquared <-
  sim_results %>%
  summarize(
    `Original data` = mean(X4),
    `New data` = mean(X5),
    `Cross-validation` = mean(X6)
  ) 

mean_rsquared_t = as.data.frame(t(mean_rsquared)) %>%
  rename(`Root mean squared error`=V1)

kable(mean_rsquared_t, caption="Root mean squared error from cross-validation and new data, showing that cross-validation provides a reasonable estimate of the model's performance on new data.")
```
