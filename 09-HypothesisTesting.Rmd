...
```{r coinFlips,echo=FALSE,fig.cap="Distribution of numbers of heads (out of 100 flips) across 100,000 simulated runs."}
...
```{r squatPlot,echo=FALSE,fig.cap="Box plots of simulated squatting ability for football players and cross-country runners."}
...
```{r scramPlot,echo=FALSE,fig.cap="Box plots for subjects assigned to each group after scrambling group labels."}
...
```{r shuffleHist, echo=FALSE,fig.cap="Histogram of differences between the football and cross-country groups after randomly shuffling group membership.  The red line denotes the actual difference observed between the two groups, and the blue line shows the theoretical t distribution for this analysis."}
...
```{r simDiff,echo=FALSE,fig.cap="Histogram of t statistics after shuffling of group labels, with the observed value of the t statistic shown in the blue line, and values more extreme than the observed value shown in orange."}
...
```{r sigResults, echo=FALSE,fig.cap="The proportion of signifcant results for a very small change (10 ounces, which is about .01 standard deviations) as a function of sample size."}
...
```{r nullSim,echo=FALSE,fig.cap="A histogram of the number of significant results in each set of 1 million statistical tests, when there is in fact no true effect."}
...
print(sprintf('corrected familywise error rate: %0.3f',mean(corOutcome>0)))

```

```{r bonferroniSim,echo=FALSE,fig.cap="A histogram of the number of significant results across all simulation runs after applying the Bonferroni correction for multiple tests."}
