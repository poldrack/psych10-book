...
For step 3, we need to specify our null hypothesis (which we call $H_0$) and our alternative hypothesis (which we call $H_A$).  $H_0$ is the baseline against which we test our hypothesis of interest: that is, what would we expect the data to look like if there was no effect?  The null hypothesis always involves some kind of equality (=, $\le$, or $\ge$).  
$H_A$ describes what we expect if there actually is an effect.  The alternative hypothesis always involves some kind of inequality ($\ne$, >, or <).  
...
Here we can see that the probability computed via simulation (.00004) is very close to the theoretical probability (.00005).  
...
Here we see that using the corrected alpha level of 0.000005 instead of the nominal level of .05, we have now controlled the familywise error, such that the probability of making *any* errors in our study is controlled at .05.
