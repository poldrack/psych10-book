...
From the plot it's clear that there is a large difference between the two groups.  We can do a standard t-test to test our hypothesis, using the `t.test()` command in R:
...
This shows that the likelihood of such a difference under the null hypothesis is very small, using the *t* distribution to define the null.   Now let's see how we could answer the same question using randomization.  The basic idea is that if the null hypothesis of no difference between groups is true, then it shouldn't matter which group one comes from (football players versus cross-country runners) -- thus, to create data that are like our actual data but also conform to the null hypothesis, we can randomly reorder the group labels for the individuals in the dataset, and then recompute the difference between the groups. Let's do this once: 
...
We can now look at the distribution of mean differences across the shuffled datasets. \@ref(fig:shuffleHist) shows the histogram of the group differences across all of the random shuffles.  As expected under the null hypothesis, this distribution is centered at zero.  
...
There are `r I(equalSum)` shuffle runs on which the t statistic for the shuffled data was exactly the same as the observed data -- which means that shuffling resulted in the same labeling as the actual data!  This is unlikely, but not *that* unlikely, and we can actually compute its likelihood using a bit of probability theory.  The number of possible permutations of 10 items is $10!$ which comes out to `r I(factorial(10))`.  The number of possible rearrangements of each set of 5 is $5!$ which comes out to `r I(factorial(5))`, so the number of possible rearrangements of two sets of five is $5! * 5!$ or `r I(factorial(5)**2)`.  Thus, we expect that  `r I((factorial(5)**2)/factorial(10))` of the random labelings should come out exactly the same as the original, which is fairly close to the `r I(sum(shuffleDiff==tt$statistic)/length(shuffleDiff))` that we see in our simulation.
...
This p-value (`r I(pvalRandomization)`) is slightly larger than the p-value that we obtained using the t distribution (`r I(sprintf('%0.5f',tt$p.value))`), though both are still quite extreme, suggesting that the observed data are very unlikely to have arisen if the null hypothesis is true - and in this case we know that it's *not* true, because we generated the data.


##### Randomization: BMI/activity example

Now let's use randomization to compute the p-value for the BMI/activity example. In this case, we will randomly shuffle the PhysActive labels and compute the difference in each shuffle, and then compare our observed t statistic to the distribution of t statistics from the shuffled datasets.
...
To understand the Neyman-Pearson framework we first need to discuss statistical decision making in terms of the kinds of outcomes that can occur.  There are two possible states of reality ($H_0$ is true, or $H_0$ is false), and two possible decisions (reject $H_0$, or fail to reject $H_0$).  There are two ways in which we can make a correct decision:
...
No. This would be $P(H_0\ is\ true|data)$, but remember as above that p-values are probabilities of data under $H_0$, not probabilities of hypotheses.
...
No.  There is a distinction between statistical significance and practical significance.  As an example, let's say that we performed a randomized controlled trial to examine the effect of a particular diet on body weight, and we find a statistically significant effect at p<.05.  What this doesn't tell us is how much weight was actually lost, which we refer to as the *effect size*.  Let's run a simulation that shows us how likely we are to find a statistically significant effect depending on two factors: the amount of weight loss, and the size of the sample.

```{r}
weightLossTrial = function(nPerGroup,effectSize){
  # mean and SD in Kg based on NHANES adult dataset
  kgToLbs=2.2046 # conversion constant for Kg to Lbs
  meanLbs=81.78*kgToLbs
  sdLbs=21.29*kgToLbs
  # create data
  controlGroup=rnorm(nPerGroup)*sdLbs + meanLbs
  expGroup=rnorm(nPerGroup)*sdLbs + meanLbs + effectSize*sdLbs
  ttResult=t.test(expGroup,controlGroup)
  return(c(nPerGroup,effectSize,ttResult$p.value,diff(ttResult$estimate)))

}

nRuns = 1000
sampSizes=2**seq(4,14) # powers of 2
effectSizes=seq(0,.05,0.01) # standardized effect sizes
kgToLbs=2.2046 # conversion constant for Kg to Lbs
effectSizesInLbs=effectSizes*21.29*kgToLbs
 
nSigResults = matrix(NA,nrow=length(sampSizes),ncol=length(effectSizes))
meanDiff = matrix(NA,nrow=length(sampSizes),ncol=length(effectSizes))

for (i in 1:length(sampSizes)){
  for (j in 1:length(effectSizes)){
    simResults = replicate(nRuns,weightLossTrial(sampSizes[i],
                                                effectSizes[j]))
    nSigResults[i,j]=sum(simResults[1,]<0.05)
    meanDiff[i,j]=mean(simResults[2,])*-1
  }
}
nSigResults=nSigResults/nRuns

```

Let's visualize the results of the simulation by presenting them as a *heatmap* where the color reflects the value of the result:

```{r}
nSigDf=data.frame(nSigResults)
names(nSigDf) = effectSizes
nSigDf = nSigDf %>%  
  mutate(sampSize = sampSizes)
nSigDfTidy = nSigDf %>% melt()
ggplot(data.frame(nsig=nSigResults),aes())
```
...
Let's imagine what would happen if the researchers simply asked whether the test was significant at p<.05 at each location.  To do this, we generate a large number of simulated t values from a null distribution, and ask how many of them are significant at p<.05.  Let's do this many times, and each time count up how many of the tests come out as significant
...
Here we see that using the corrected alpha level of `r I(corAlpha)` instead of the nominal level of .05, we have now controlled the familywise error, such that the probability of making *any* errors in our study is controlled at .05.
