...
powerSimResults = powerDf %>% do(runPowerSim(.))


```

```{r plotPowerSim,fig.cap="Results from power simulation, showing power as a function of sample size, with effect sizes shown as different colors, and alpha shown as line type. The standard criterion of 80% power is shown by the dotted black line."}
...
This tells us that we would need about 64 subjects in each group in order to have sufficient power to find a medium-sized effect.  It's always important to run a power analysis before one starts a new study, to make sure that the study won't be futile due to a sample that is too small.

```{r echo=FALSE}
p = pwr.t.test(d=3,power=0.8,sig.level=0.05)
```
