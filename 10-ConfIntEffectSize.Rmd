...
```{r CISampSize,echo=FALSE,fig.cap="An example of the effect of sample size on the width of the confidence interval for the mean."}
ssDf <- 
  tibble(sampSize=2**seq(4,9)) %>%
  mutate(
...
data.frame(lower=bootci$perc[4],
           upper=bootci$perc[5]) %>%
  pander()
```
...
An effect size is a standarized measurement that compares the size of some statistical effect to a reference quantity, such as the variability of the statistic. In some fields of science and engineering this idea is referred to as a "signal to noise ratio".  There are many different ways that this can be quantified, which depend on the nature of the data.
...
```{r genderHist,echo=FALSE,fig.cap="Smoothed histogram plots for male and female heights in the NHANES dataset, showing clearly distinct distributions."}
ggplot(NHANES_sample,aes(x=Height,color=Gender)) + 
  geom_density()
```

It is also worth nothing that we rarely encounter effects of this magnitude in science, in part because they are such obvious effects that we don't need scientific research to find them.  As we will see in the later chapter on reproducibility, very large reported effects in scientific research often reflect the use of questionable research practices rather than truly huge effects in nature. 
...
```{r corrFig,echo=FALSE,fig.cap="Examples of various levels of Pearson's r."}
set.seed(123456789)
p <- list()
...
- Type I error rate: There is a tradeoff between Type I error and power, such that (all else being equal) decreasing Type I error will also decrease power.
...
powerDf <- 
  expand.grid(sampSizePerGroup = c(12,24,48,96),
              effectSize=c(.2,.5,.8),
              alpha=c(0.005,0.05)
    ) %>%
...
runPowerSim <- function(df,nsims=1000){
  ...
  for (s in 1:nsims){
    data <- data.frame(y=rnorm(df$sampSizePerGroup*2),
...
```{r plotPowerSim,echo=FALSE,fig.cap="Results from power simulation, showing power as a function of sample size, with effect sizes shown as different colors, and alpha shown as line type. The standard criterion of 80% power is shown by the dotted black line."}
...
There are at least two important reasons to care about statistical power, one of which we discuss here and the other of which we will return to in the final chapter.  If you are a researcher, you probably don't want to spend your time doing futile experiments.  However, running an underpowered study is essentially futile, because it means that there is a very low likelihood that one will find an effect, even if it exists.  

### Power analysis

Fortunately, there are tools available that allow us to determine the statistical power of an experiment. The most common use of these tools is in planning an experiment, when we would like to determine how large our sample needs to be in order to be adqequately powered to find our effect of interest.
...
It might have occurred to you that if the effect size is large enough, then the necessary sample will be very small.  For example, if we run the same power analysis with an effect size of d=3, then we will see that we only need about 3 subjects in each group to have sufficient power to find the difference.  However, it's rare in science to be doing an experiment where we expect to find such a large effect -- just as we don't need statistics to tell us that 16-year-olds are taller than than 6-year-olds.  When we run a power analysis, we need to specify an effect size that is plausible for our study, which would usually come from previous research.  However, in the final chapter we will discuss a phenomenon known as the "winner's curse" that likely results in published effect sizes being larger than the true effect size, so this should also be kept in mind.  
