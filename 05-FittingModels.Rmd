...
One of the fundamental activties in statistics is creating models that can summarize data using a small set of numbers, thus providing a compact description of the data.  In this chapter we will discuss the concept of a statistical model and how it can be used to describe data.
...
What we notice here is that the average individual has a pretty large error of `r I(mean(error_mode))` centimeters. We would like to have a model where the average error is zero, and it turns out that if we use the arithmetic mean (commonly known as the *average*) as our model then this will be the case. 
...
The average error here is a very small number, though not technically zero; we will discuss later in the course why this happens (which is due to how computers represent numbers), but for now you can just treat it as being close enough to zero to call it zero.
...
This shows that the mean has a pretty substantial amount of error -- any data point will be about 27 cm from the mean on average -- but it's still much better than the mode. 
...
```{r childAgeHeight, echo=FALSE,fig.cap="Height of children in NHANES, plotted as a function of age."}
...
The black points in Figure \@ref(fig:childAgeHeight) show individuals in the dataset, and there seems to be a strong relationship between height and age, as we would expect.  Thus, we might build a model that relates height to age:
...
where $\beta$ is a *parameter* that we multiply by age to get the smallest error.  You might have noticed that this is simply a line with a slope of $\beta$ - to see this, let's plot the best fitting line on top of the data (Figure \@ref(fig:childHeightLine)).

```{r childHeightLine,echo=FALSE,fig.cap="Height of children in NHANES, plotted as a function of age, with the best fitting model using only height showing in blue. Notice that the line systematically misses the data points."}
...
Somethis is clearly wrong with this model, as the line doesn't seem to follow the data very well.  In fact, the RMSE for this model (`r I(rmse_heightOnly)`) is actually higher than the model that only includes the mean! The problem comes from the fact that our model only includes age, which means that the predicted value of height from the model must take on a value of zero when age is zero.  Even though the data do not include any children with an age of zero, the line is mathematically constrained to have a y-value of zero when x is zero, which explains why the line is pulled down below the younger datapoints.  We can fix this by including a constant value in our model, which basically represents the estimated value of height when age is equal to zero; even though an age of zero is not plausible in this dataset, this is a mathematical trick that will allow the model to account for the overall magnitude of the data.  The model is:

$$
height_i = constant + \beta * age_i + error_i
$$

where *constant* is a constant value added to the prediction for all individuals (which we also call the *intercept*, since it maps onto the intercept in the equation for a line).  We will also learn later how it is that we actually compute these values; for now, we will use the `geom_smooth()` function in R to compute the values of the constant and $\beta$ that give us the smallest error.  Figure \@ref(fig:childHeightConstLine) shows this model fitted to the NHANES data, where we see that the line matches the data much better than the one without a constant. 

```{r childHeightConstLine,echo=FALSE,fig.cap="Height of children in NHANES, plotted as a function of age, with the best fitting model including both age and a constabnt shown in blue."}
...
```{r ageHeightError,echo=FALSE,fig.cap="Distribution of errors from model including constant and age."}
...
```{r heightAgeGender,echo=FALSE,fig.cap="Height plotted separately for boys and girls.  The two lines provide a summary for each group."}
...
From the plot, it seems that there is a difference between males and females, but it is relatively small and only emerges after the age of puberty.  Let's estimate this model and see how the errors look:
...
```{r msePlot,echo=FALSE, fig.cap="Mean squared error plotted for each of the models tested above."}
...
```{r, BACrt,echo=FALSE,fig.cap="Simulated relationship between blood alcohol content and reaction time on a driving test."}
dataDf <-  
  tibble(
    BAC = runif(100) * 0.3,
...
```{r BACrt2,echo=FALSE,fig.cap="Simulated relationship between BAC and reaction time, with greater noise."}
dataDf <-  
  tibble(
    BAC = runif(100) * 0.3,
...
print(sprintf("mean = %0.2f", mean(NHANES_child$Height)))
...
```{r crimeHist,echo=FALSE,fig.cap="Histogram of the number of violent crimes.  The value for CA is plotted in blue."}
crimeData <- 
  read.table(
...
```{r crimeMapRaw,echo=FALSE,fig.cap="A map of the US with crime rates depicted in color.  Example adapted from https://cran.r-project.org/web/packages/fiftystater/vignettes/fiftystater.html"}
library(mapproj)
library(fiftystater)
...
```{r popVsCrime,echo=FALSE,fig.cap="A plot of number of crimes versus population by state."}
crimeData %>%
...
```{r crimeRateHist,fig.cap="A histogram of per capita crime rates, expressed as crimes per 100,000 people."}
crimeData %>% 
...
print(sprintf("rate of 2014 violent crimes in CA: %.2f", caCrimeData$Violent.Crime.rate))
print(sprintf("mean rate: %.2f", mean(crimeData$Violent.Crime.rate)))
print(sprintf("std of rate: %.2f", sd(crimeData$Violent.Crime.rate)))
...
```{r crimeZplot,echo=FALSE,fig.cap="Scatterplot of original crime rate data against Z-scored data."}
crimeData <- 
  crimeData %>%
  mutate(
...
```{r stdScores,echo=FALSE,fig.cap="Crime data presented as standardized scores with mean of  100 and standard deviation of 10."}
crimeData <-
  crimeData %>% 
  mutate(
...
```{r crimeTypePlot,echo=FALSE,fig.cap="Plot of violent vs. property crime rates.",fig.height=6, fig.width=6}
...
```{r crimeTypeZPlot,echo=FALSE,fig.cap="Z-scored values of violent versus property crime rates.",fig.height=6, fig.width=6}
crimeData <- 
  crimeData %>%
  mutate(
...
```{r crimeTypePopPlot,echo=FALSE,fig.cap="Plot of violent vs. property crime rates, with population size presented through the size of the plotting symbol; California is presented in blue."}
crimeData %>% 
  ggplot(aes(ViolentCrimeRateZscore, PropertyCrimeRateZscore)) +
...
```{r violenceDifference,echo=FALSE,fig.cap="A plot of the violence difference scores versus popluation."}
crimeData <- 
  crimeData %>%
  mutate(
