...
error_mean <- error_mode <- NHANES_child$Height - mean(NHANES_child$Height)
...
This shows that the mean has a pretty substantial amount of error -- any individual data point will be about 27 cm from the mean on average -- but it's still much better than the mode. 
...
sprintf("Variance estimate using n: %.2f", mean(varhat_n))
sprintf("Variance estimate using n-1: %.2f", mean(varhat_nm1))
...
sprintf("Mean rate across states: %.2f", mean(crimeData$Violent.Crime.rate))
sprintf("Std. deviation of rate across states: %.2f", sd(crimeData$Violent.Crime.rate))
